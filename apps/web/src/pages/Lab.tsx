...
export const Lab: React.FC<LabProps> = ({ stashedProducts, currentOutfitItems, setCurrentOutfitItems, addToFit, saveOutfit }) => {
  ...
  const grouped = useMemo(() => {
    ...
    for (const p of stashedProducts) {
      const parent = ((p as unknown as Record<string, unknown>).parentCategory as string) || '';
      ...
      const isShoe = shoeKeywords.some(k => parent.toLowerCase().includes(k) || slugCategory.includes(k) || name.includes(k));
      if (isShoe) {
        groups.SHOES.push(p);
        continue;
      }
      ...
      // Accessories
      if (parent.toLowerCase().includes('accessories') || slugCategory.includes('bag') || slugCategory.includes('belt') || slugCategory.includes('cap') || slugCategory.includes('hat') || slugCategory.includes('watch') || slugCategory.includes('sunglass')) {
        groups.ACCESSORIES.push(p);
        continue;
      }

      // Clothing/apparel -> decide TOP vs BOTTOM
      if (parent.toLowerCase().includes('apparel') || slugCategory.includes('shirt') || slugCategory.includes('t-shirt') || slugCategory.includes('hoodie') || slugCategory.includes('tops') || slugCategory.includes('pants') || slugCategory.includes('jeans')) {
        if (slugCategory.includes('pants') || slugCategory.includes('shorts') || slugCategory.includes('jeans') || slugCategory.includes('sweatpants') || slugCategory.includes('leggings')) {
