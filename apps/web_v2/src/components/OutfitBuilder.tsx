...
export const OutfitBuilder: React.FC<OutfitBuilderProps> = ({ items, setItems, onSaveOutfit }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="p-2 border-b-2 border-black flex justify-between items-center bg-gray-50">
        ...
        {selectedItem && (
          <div className="flex gap-2">
            <button onClick={() => updateItem(selectedItem.id, { rotation: (selectedItem.rotation || 0) + 45 })} className="p-2 border-2 border-black bg-white hover:bg-gray-100" title="Rotate">
              <Icon icon="lucide:rotate-cw" width="16" height="16" />
            </button>
            <button onClick={() => updateItem(selectedItem.id, { scale: (selectedItem.scale || 1) + 0.1 })} className="p-2 border-2 border-black bg-white hover:bg-gray-100" title="Zoom In">
              <Icon icon="lucide:zoom-in" width="16" height="16" />
            </button>
            <button onClick={() => updateItem(selectedItem.id, { scale: Math.max(0.2, (selectedItem.scale || 1) - 0.1) })} className="p-2 border-2 border-black bg-white hover:bg-gray-100" title="Zoom Out">
              <Icon icon="lucide:zoom-out" width="16" height="16" />
            </button>
            <button onClick={() => updateItem(selectedItem.id, { zIndex: (selectedItem.zIndex || 1) + 1 })} className="p-2 border-2 border-black bg-white hover:bg-gray-100" title="Bring Forward">
              <Icon icon="lucide:move" width="16" height="16" />
            </button>
            <button onClick={() => removeItem(selectedItem.id)} className="p-2 border-2 border-black bg-red-100 hover:bg-red-200 text-red-600" title="Remove">
        ...
        {items.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
            <h2 className="font-display text-4xl text-gray-400 uppercase text-center">
              The Lab is Empty<br/>
              <span className="text-lg font-mono">Add items from your stash</span>
            </h2>
          </div>
        )}
